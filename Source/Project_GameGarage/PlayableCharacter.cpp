...
{
	if (HasAuthority())  // 서버에서만 실행되도록 보장
	{
	...
	//	FName HitBoneName = SweepResult.BoneName;  // 충돌된 본 이름 가져오기
	//	if (!HitBoneName.IsNone())
	//	{
	//		UE_LOG(LogTemp, Warning, TEXT("IsNone"));
	//		// 해당 본에 대해 물리 시뮬레이션 활성화
	//		GetMesh()->SetAllBodiesBelowSimulatePhysics(HitBoneName, true, false);
	//		// 충돌된 부위에 임펄스 추가 (예: 더 사실적인 반응을 위해)
	//		FVector Impulse = SweepResult.ImpactNormal * 100.0f;  // 예: 임펄스 강도 조절
...
void APlayableCharacter::LoadAndApplySavedData()
{
	...
	if (SaveGameObject)
	{
		...
		// 불러온 세이브 데이터에서 Mesh와 EmoMaterial 설정
